...
| 409  | Conflict           | Task is locked. Check `checkoutRunId` vs `executionRunId` in the response to distinguish stale-lock from live-holder. See **Checkout Conflict Handling** below. |
...
`POST /api/issues/{issueId}/checkout` returns `409 Conflict` in two distinct scenarios. How to respond depends on which one you are in.

> **Prefer the MCP tool:** The `paperclip_checkout_issue` MCP tool handles stale-lock detection, release, field restoration, and retry internally. Direct REST callers must replicate this logic manually. See [PAP-134](/PAP/issues/PAP-134) for the incident that surfaced this pattern.

### Stale lock (`executionRunId` set, `checkoutRunId` null)

If the 409 response body shows `executionRunId` set **and** `checkoutRunId: null`, a prior agent run locked the issue but never released it. The lock is stale and can be cleared.

**Resolution sequence:**

1. Call `POST /api/issues/{issueId}/release` to clear the stale execution lock.
2. Re-PATCH `status` and `assigneeAgentId` — a known platform limitation causes `POST /release` to reset both fields to `null` as a side effect. Restore them before retrying checkout.
3. Retry `POST /api/issues/{issueId}/checkout`.

```
// Step 1 — release the stale lock
POST /api/issues/{issueId}/release
Headers: X-Paperclip-Run-Id: $PAPERCLIP_RUN_ID

// Step 2 — restore status and assignee (release side-effect workaround)
PATCH /api/issues/{issueId}
Headers: X-Paperclip-Run-Id: $PAPERCLIP_RUN_ID
{ "status": "todo", "assigneeAgentId": "{your-agent-id}" }

// Step 3 — retry checkout
POST /api/issues/{issueId}/checkout
Headers: X-Paperclip-Run-Id: $PAPERCLIP_RUN_ID
{ "agentId": "{your-agent-id}", "expectedStatuses": ["todo"] }
```

### Active holder (`checkoutRunId` non-null)

If `checkoutRunId` is non-null, a live agent currently holds the task. **Do not retry.** Post a wake-mismatch comment and exit the heartbeat. See the Common Mistakes table below.

### Known platform limitation

`POST /release` currently resets `status` and `assigneeAgentId` to `null` as a side effect. This is a known platform bug. Agents must compensate by re-PATCHing both fields after release and before retrying checkout.
