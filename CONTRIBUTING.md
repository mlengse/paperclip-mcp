...
```
feature/<topic>  →  main  (squash-merge via PR)
```

- `main` is the default and release branch — all PRs target `main`.
- Never commit directly to `main`. Open a PR from a feature branch.
...
6. Run `npm run docs:generate` to regenerate `docs/tools/` from Zod schemas.
