...
| Peer Agent      | Boundary                                                                                                                                                                                                                                                                                             |
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Engineer        | Engineer implements code and merges to `main`. Release Manager confirms which Engineer commits are in scope for the release and drafts the changelog entry — does not review code quality (that is QA).                                                                                              |
...
| DevOps Engineer | DevOps owns `release.yml`, `.releaserc.json`, and the semantic-release execution. Release Manager owns everything upstream of that automation: notes, coordination, and the decision to merge `main` to `main`. Release Manager requests the merge; DevOps confirms the pipeline will run correctly. |
...
Draft the v1.0.1 release notes template at `docs/releases/v1.0.1.md` based on commits on `main` since the v1.0.0 tag (`git log v1.0.0..main --oneline --no-merges`). Group entries into Features, Fixes, and Internal. Commit the file and update `RELEASES.md` with the v1.0.1 entry (status: draft).
...
6. After approval, assign the probe issue ("Draft the v1.0.1 release notes template based on changes in the current main branch").
