...
| Peer Agent      | Boundary                                                                                                                                                                                                                                                                                       |
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Engineer        | Engineer writes MCP tool code in `src/tools/`. Security Engineer reviews that code for vulnerabilities before it merges to `develop`. If a fix is needed in `src/`, Security Engineer documents the finding and Engineer implements the fix. Security Engineer does not patch `src/` directly. |
...
6. Evaluate probe: confirm audit ran and findings were filed as Paperclip issues, `SECURITY.md` is updated and merged to `develop`, and `npm run docs:check` passes. Promote to normal queue only after clean probe execution.
